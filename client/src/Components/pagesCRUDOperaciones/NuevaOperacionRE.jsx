...
function NuevaOperacionRE({ tipo }) {
    ...
    const handleGuardarProductos = async () => {
        ...
        try {
            const response =
                await operacionProductoApi.handleChangeProductsRequest(
                    operacion.id,
                    productos
                )
    ...
    const filterProductos = () => {
        ...
        const filtered = loadedProductos.filter(
            (producto) =>
                !productos.some((item) => item.producto === producto.nombre)
        )
    ...
    const handleCrearOperacion = async (atributos) => {
        ...
        try {
            const response = await operacionApi.createOperacionRequest(
                operacion
            )
            ...
            setSelectedAlmacen(
                response.data.Data.to ? response.data.Data.to : ''
            )
