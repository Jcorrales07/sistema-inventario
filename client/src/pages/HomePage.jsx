...
function HomePage() {
  ...
  const [topProductos, setTopProductos] = useState([
    ...
    { nombre: "Producto 4", cantidad: 40 },
    { nombre: "Producto 5", cantidad: 50 },
    { nombre: "Producto 6", cantidad: 60 },
  ]);
  ...
  const loadTopProductos = async () => {
    try {
      const response = await operacionApi.getInventarioOperacionRequest();
      ...
      const productosSorted = productos.sort((a, b) => b.salidas - a.salidas);

      const topProductos = [];

      for (let i = 0; i < 8; i++) {
        if (productosSorted[i] === undefined) {
          break;
        }

        if (productosSorted[i].salidas === 0) {
         break;
        }

        topProductos.push({
          nombre: productosSorted[i].nombre,
          cantidad: productosSorted[i].salidas,
        });
      }

      setTopProductos(topProductos);
    } catch (error) {
  ...
  const fetchData = async () => {
    try {
      ...
      const recibidos = response2.data.Data.filter((item) => item.tipo === 0);
      const entregas = response2.data.Data.filter((item) => item.tipo === 1);
      ...
      <>
        <Row className="text-center mb-3">
          {topProductos.map((producto) => (
            <Col className="mx-5 my-3" style={{height: "180px"}}>
              <Card className="border-0 shadow-lg py-4 px-5" style={{height: "180px"}}>
                <Card.Body className="d-flex flex-column">
                  <h5 style={{ flexGrow: 1 }}>{producto.nombre}</h5>
                  <p className="display-6">{producto.cantidad}</p>
                </Card.Body>
              </Card>
            </Col>
          ))}
      ...
      <Row>
        ...
        <Col className="flex-grow-1 p-0">
          ...
          >
              ...
              <Col className="mx-2">
                <Card className="border-0 shadow-sm p-3">
                  <Card.Body>
                    ...
                    <p className="display-6">{productCount}</p>
                  </Card.Body>
                </Card>
              </Col>
              <Col className="mx-2">
                <Card className="border-0 shadow-sm p-3">
                  <Card.Body>
                    <h5>
                      <i className="bi bi-arrow-down-circle"></i> Cantidad de
                      recepciones
                    </h5>
                    <p className="display-6">{recibidosCount}</p>
                  </Card.Body>
                </Card>
              </Col>
              <Col className="mx-2">
                <Card className="border-0 shadow-sm p-3">
                  <Card.Body>
                    <h5>
                      <i className="bi bi-arrow-up-circle"></i> Cantidad de
                      entregas
                    </h5>
                    <p className="display-6">{entregasCount}</p>
                  </Card.Body>
                </Card>
              </Col>
            </Row>
            ...
            <h4 className="mb-2">Productos mas vendidos</h4>
